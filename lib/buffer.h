...
typedef struct buffer {
  ...
  unsigned long n;	/* current size of string in buffer */
  unsigned long a;	/* allocated buffer size */
  int fd;		/* passed as first argument to op */
  long (*op)();		/* use read(2) or write(2) */
...
void buffer_init(buffer* b,long (*op)(),int fd,char* y,unsigned long ylen);
void buffer_init_free(buffer* b,long (*op)(),int fd,char* y,unsigned long ylen);

void buffer_default(buffer *b, long (*op)());
...
int buffer_put(buffer* b,const char* x,unsigned long len);
int buffer_putc(buffer* b,char c);
int buffer_putalign(buffer* b,const char* x,unsigned long len);
int buffer_putflush(buffer* b,const char* x,unsigned long len);
...
int buffer_get(buffer* b,char* x,unsigned long len);
int buffer_feed(buffer* b);
int buffer_getc(buffer* b,char* x);
int buffer_getn(buffer* b,char* x,unsigned long len);

/* skips bytes in buffer until a char in charset occurs, the char itself will be skipped also */
int buffer_skip_until(buffer* b,const char* charset,unsigned long setlen);  
...
typedef int (*string_predicate)(const char* x,unsigned long len);

/* like buffer_get_token but the token ends when your predicate says so */
int buffer_get_token_pred(buffer* b,char* x,unsigned long len,string_predicate p);
