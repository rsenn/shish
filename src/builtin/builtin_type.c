  ...
  for(i = 0; i < EXEC_HASHSIZE; i++) {
    for(h = exec_hashtbl[i]; h; h = h->next) {
      ...
      switch(h->id) {
        ...
        case H_EXEC:
        case H_SBUILTIN: buffer_putm(fd_out->w, h->name, " (special builtin)", NULL); break;
        case H_BUILTIN: buffer_putm(fd_out->w, h->name, " (builtin)", NULL); break;
        case H_FUNCTION: buffer_putm(fd_out->w, h->name, " (function)", NULL); break;
      }
