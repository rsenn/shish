...
void debug_subst(const char *msg, int flags, int depth) {
  ...
  if(flags & S_BQUOTE)  n += str_copy(&flagstr[n], "|S_BQUOTE");
  if(flags & S_STRLEN)  n += str_copy(&flagstr[n], "|S_STRLEN");
  if(flags & S_NULL)    n += str_copy(&flagstr[n], "|S_NULL");
  if(flags & S_NOSPLIT) n += str_copy(&flagstr[n], "|S_NOSPLIT");
