...
const char* debug_nodes[] = {
    "N_SIMPLECMD",  "N_PIPELINE",  "N_AND",      "N_OR",       "N_NOT",          "N_SUBSHELL",
    "N_CMDLIST",    "N_FOR",       "N_CASE",     "N_CASENODE", "N_IF",           "N_WHILE",
    "N_UNTIL",      "N_FUNCTION",  "N_ARG",      "N_ASSIGN",   "N_REDIR",        "N_ARGSTR",
    "N_ARGCMD",     "N_ARGPARAM",  "N_ARGARITH", "A_NUM",      "A_PAREN",        "A_OR",
    "A_AND",        "A_BOR",       "A_BXOR",     "A_BAND",     "A_EQ",           "A_NE",
    "A_LT",         "A_GT",        "A_GE",       "A_LE",       "A_LSHIFT",       "A_RSHIFT",
    "A_ADD",        "A_SUB",       "A_MUL",      "A_DIV",      "A_MOD",          "A_EXP",
    "A_UNARYMINUS", "A_UNARYPLUS", "A_NOT",      "A_BNOT",     "A_PREDECREMENT", "A_PREINCREMENT"};

void
debug_node(union node* node, int depth) {
  const char* name = debug_nodes[node->id];

  debug_space(depth, depth > 0 ? 1 : 0);
  ...
  case N_ARGPARAM: {
    ...
    buffer_puts(fd_err->w, COLOR_LIGHTBLUE "${");
