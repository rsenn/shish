...
/* debugs a tree node!
 * ----------------------------------------------------------------------- */
const char* debug_nodes[] = {"N_SIMPLECMD", "N_PIPELINE", "N_AND",        "N_OR",        "N_NOT",      "N_SUBSHELL", "N_CMDLIST",      "N_FOR",
                             ...
                             "A_MOD",       "A_EXP",      "A_UNARYMINUS", "A_UNARYPLUS", "A_NOT",      "A_BNOT",     "A_PREDECREMENT", "A_PREINCREMENT"};
...
debug_node(union node* node, int depth) {
  ...
  switch(node->id) {
    ...
    case N_ARGSTR:
      ...
      debug_stralloc(0, &node->nargstr.stra, depth, node->nargstr.flag & S_DQUOTED ? '"' : node->nargstr.flag & S_SQUOTED ? '\'' : '\0');
      break;
