...
#
# -DBUILTIN_<NAME>=ON|OFF     that builtin; permanent (it stays in the cache) -DENABLE_ALL_BUILTINS=ON    every builtin that has no BUILTIN_<NAME> of its own; permanent, -DENABLE_ALL_BUILTINS=OFF undoes it neither DEFAULT_BUILTINS (dump:
# also in debug builds)
#
# BUILTIN_<NAME> is AUTO until somebody sets it, which is what tells an answer from a default -- the computed result is kept in a plain variable of the same name and never written back to the cache, so a later configure (also the one cmake
# starts by itself when a CMake file changes) reaches the same decision.
option(ENABLE_ALL_BUILTINS "Build every builtin (BUILTIN_<NAME>=OFF still wins)" OFF)

# Build directories from before this scheme cached the *computed* answer in BUILTIN_<NAME>; those are not choices, so forget them once. The BUILD_BUILTIN_* entries are written by every configure, old or new.
...
foreach(BUILTIN ${ALL_BUILTINS})
  ...
  # a plain truth test: isin() answers TRUE/FALSE and an option() answers ON/OFF, and "TRUE STREQUAL ON" is false
  if(BUILTIN_${NAME})
...
# now that BUILTINS_ENABLED is populated, "is dump being built" is a real answer -- and dump's output goes through the debug buffer, so building it turns DEBUG_OUTPUT on by default (CMakeLists.txt).
...
# NAME has to come from ${DISABLED}: without it the loop reuses whatever NAME the previous foreach left behind, so a builtin that was enabled in an earlier configure of this build directory kept its cached BUILD_BUILTIN_<NAME>=1 and stayed
# in builtin_config.h.
